...
import type { IdOrNameProps, ProjectScopeProps } from "../types.js";
...
export const builder = (argv: yargs.Argv) =>
	argv
			...
			(yargs) =>
				yargs
					...
					.options({
						"preserve-under-name": {
							describe:
								"Name under which to preserve the old branch",
						},
					})
					.example([
						[
		...
		.command({
			command:
				"schema-diff [base-branch] [compare-source[@(timestamp|lsn)]]",
			aliases: ["sd"],
			describe:
...
const create = async (
	...
	if (props.psql) {
		if (!data.connection_uris?.length) {
			throw new Error(
				`Branch ${data.branch.id} doesn't have a connection uri`,
			);
		}
